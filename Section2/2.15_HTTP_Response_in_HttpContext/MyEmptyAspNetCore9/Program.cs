...
app.Run(async (HttpContext context) =>
{
    ...
    else if (context.Request.Path.StartsWithSegments("/employees"))
    {
        if (context.Request.Method == "GET")
        {
            ...
            context.Response.StatusCode = 200; // GET request successful 200 OK, default
        }
        else if (context.Request.Method == "POST")
        {
            ...
            context.Response.StatusCode = 201; // POST request successful 201 Created
        ...
        else if (context.Request.Method == "PUT")
        {
            ...
            if (result)
            {
                context.Response.StatusCode = 204; // PUT request successful 204 No Content
                await context.Response.WriteAsync("Employee updated successfully.");
                return;
            }
            else
            {
                context.Response.StatusCode = 404; // Employee not found
            ...
            if (context.Request.Query.ContainsKey("id"))
            {
                ...
                if (int.TryParse(id, out int employeeId))
                {
                    if (context.Request.Headers["Authorization"] == "frank")
                    {
                        ...
                        if (result)
                        {
                            context.Response.StatusCode = 200; // DELETE request successful 200 OK
